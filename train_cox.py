...
parser.add_argument('--n_epochs', type=int, default=40, help='train epoch')
...
parser.add_argument('--omics_type',type=str,default='TCGA-KIRC')
...
def train(config,dataloader,dataloader_val,num_epochs, batch_size, learning_rate,  measure, verbose,save_state,time_str,idx):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i,data in enumerate(dataloader):
            ...
            survtime=data['time']
            lbl=data['event'].cuda()
            lbl_pred,code,aux_loss=model(exp)
            if iter == 0:
